...
class ImplantDataset(Dataset):
    def __init__(self, json_path, bin_edges, transform=None, image_root=None):
        ...
        if diameters:
            ...
            print(f"   Samples: {len(diameters)}")
            print(f"   Range: {min(diameters):.1f} - {max(diameters):.1f} mm")
            print(f"   Mean: {np.mean(diameters):.1f} mm")
            print(f"   Std: {np.std(diameters):.1f} mm")
            
            # Show bin distribution
            bin_counts = np.histogram(diameters, bins=self.bin_edges)[0]
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if diameter > 0:
                # Find which bin the diameter belongs to
                # np.digitize returns 0 if x < bins[0], n if x >= bins[n-1]
                label = np.digitize(diameter, self.bin_edges) - 1
...
class DiameterWithCompanyClassifier(nn.Module):
    def __init__(self, company_checkpoint_path, num_diameter_classes, use_attention=True):
        super().__init__()
        ...
        # Diameter classification head (uses both features and company embeddings)
        ...
        self.diameter_head = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(256, num_diameter_classes)
        )
        
        print(f"🏗️  Diameter with Company Classifier:")
        print(f"   Company embeddings: {self.embed_dim}")
        print(f"   Diameter bins: {num_diameter_classes}")
    ...
    def forward(self, x):
        ...
        diameter_logits = self.diameter_head(combined)
        
        return diameter_logits

def create_diameter_bins(entries, bin_size=0.5):
    """Create diameter bins based on data distribution"""
    diameters = []
    for e in entries:
        if e.get("diameter") is not None:
            try:
    ...
    if not diameters:
        raise ValueError("No valid diameter values found in dataset")
    ...
    buffer = bin_size * 0.1
    min_bin = np.floor((min_d - buffer) / bin_size) * bin_size
    max_bin = np.ceil((max_d + buffer) / bin_size) * bin_size
    
    # Create bin edges
    bin_edges = np.arange(min_bin, max_bin + bin_size, bin_size)
    
    print(f"📏 Created {len(bin_edges)-1} diameter bins")
    print(f"   Bin size: {bin_size} mm")
    print(f"   Range: {min_bin:.1f} - {max_bin:.1f} mm")
    print(f"   Data range: {min_d:.1f} - {max_d:.1f} mm")
    print(f"   Number of unique values: {len(set(diameters))}")
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train Diameter Classifier with Company Embeddings')
    ...
    parser.add_argument('--output_dir', type=str, default='outputs_diameter_company')
    ...
    parser.add_argument('--bin_size', type=float, default=0.2,
                        help='Bin size in mm for diameter classification')
...
def main():
    ...
    print("📏 Diameter Classification with Company Embeddings")
    print("=" * 60)
    ...
    print(f"\n📏 Creating diameter bins from {len(all_entries)} total entries")
    bin_edges = create_diameter_bins(all_entries, bin_size=args.bin_size)
    num_classes = len(bin_edges) - 1
    ...
    model = DiameterWithCompanyClassifier(
