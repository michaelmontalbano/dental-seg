...
Now supports ResNet18/ResNet50 backbone selection
"""
...
class TransUNet(nn.Module):
    """Pure TransUNet for landmark segmentation with configurable backbone"""
    def __init__(self, num_classes=6, img_size=1024, patch_size=16, embed_dim=768, depth=12, num_heads=12, backbone='resnet50'):
        super().__init__()
        ...
        # CNN Encoder - Get proper channel dimensions for each backbone
        import torchvision.models as models
        
        # Define channel dimensions for each ResNet variant
        if backbone == 'resnet18':
            resnet = models.resnet18(pretrained=True)
            # ResNet18 actual output channels per layer
            self.layer3_channels = 256  # layer3 outputs 256 channels
            self.skip1_channels = 64   # layer1 output
            self.skip2_channels = 128  # layer2 output
        else:  # resnet50
            resnet = models.resnet50(pretrained=True)
            # ResNet50 actual output channels per layer
            self.layer3_channels = 1024  # layer3 outputs 1024 channels
            self.skip1_channels = 256  # layer1 output
            self.skip2_channels = 512  # layer2 output
        ...
        self.layer1 = resnet.layer1  # Output: skip1_channels
        self.layer2 = resnet.layer2  # Output: skip2_channels
        self.layer3 = resnet.layer3  # Output: layer3_channels
        
        # Patch embedding for transformer
        feature_size = img_size // 16  # 64 for 1024 input, 32 for 512 input
        self.feature_size = feature_size  # Store for later use
        
        self.patch_embed = PatchEmbed(
            img_size=feature_size, 
            patch_size=4,  # 4x4 patches on feature map
            in_chans=self.layer3_channels,  # Use the correct layer3 output channels
            embed_dim=embed_dim
        )
        
        # Positional embedding
        num_patches = (feature_size // 4) * (feature_size // 4)  # Adjusted for 4x4 patches
        ...
        # Decoder - carefully match dimensions with encoder skip connections
        # First: transformer output to intermediate dimension
        self.decoder_patch = nn.ConvTranspose2d(embed_dim, embed_dim, kernel_size=4, stride=4)
        
        # Then progressively upsample, matching skip connection dimensions
        # decoder4: embed_dim -> 256, then concat with layer2 output
        self.decoder4_prep = nn.Conv2d(embed_dim, 256, kernel_size=1)  # Reduce embed_dim to 256
        self.decoder4 = nn.ConvTranspose2d(256, 256, kernel_size=2, stride=2)
        
        # decoder3: concat [256 from decoder4, skip2_channels from layer2] -> 256
        self.decoder3 = nn.ConvTranspose2d(256 + self.skip2_channels, 256, kernel_size=2, stride=2)
        
        # decoder2: concat [256 from decoder3, skip1_channels from layer1] -> 128
        self.decoder2 = nn.ConvTranspose2d(256 + self.skip1_channels, 128, kernel_size=2, stride=2)
        
        # decoder1: 128 -> 64 (no skip connection)
        ...
        logger.info(f"TransUNet initialized with {backbone}:")
        logger.info(f"  Layer3 output channels: {self.layer3_channels}")
        logger.info(f"  Skip connection channels: layer1={self.skip1_channels}, layer2={self.skip2_channels}")
        logger.info(f"  Embed dim: {embed_dim}, Depth: {depth}, Heads: {num_heads}")
    ...
    def forward(self, x):
        ...
        # Reshape back to spatial (adjusted for 4x4 patches)
        patch_grid_size = self.feature_size // 4
        x_transformer = x_transformer.transpose(1, 2).reshape(B, self.embed_dim, patch_grid_size, patch_grid_size)
        
        # CNN Decoder with skip connections
        # First upsample from patch grid back to feature map size
        d_patch = self.decoder_patch(x_transformer)  # Back to 1/16 resolution
        
        # Reduce channels before upsampling
        d_patch = self.decoder4_prep(d_patch)  # embed_dim -> 256
        d4 = self.decoder4(d_patch)           # 256 -> 256, 1/8 resolution
        d4 = torch.cat([d4, x2], dim=1)       # Concat with layer2 output
        
        d3 = self.decoder3(d4)                 # -> 256, 1/4 resolution
        d3 = torch.cat([d3, x1], dim=1)       # Concat with layer1 output
        
        d2 = self.decoder2(d3)                 # -> 128, 1/2 resolution
        d1 = self.decoder1(d2)                 # -> 64, full resolution
...
class SegmentationDataset(BboxDataset):
    ...
    def __init__(self, annotations_file, images_dir, transforms=None, img_size=1024, class_group='all', xray_type=None):
...
def calculate_simplified_metrics(outputs, targets, num_classes, original_img_sizes=None, resized_img_size=512, confidence_threshold=0.4):
    """
    Calculate simplified metrics: IoU, Dice, Precision, and 2mm distance accuracy per class
    
    Args:
        original_img_sizes: List of (width, height) tuples for each image in batch
        resized_img_size: Resized image size used for training (default 512)
        confidence_threshold: Confidence threshold for distance metrics
    """
    # Default pixel spacing if no original sizes provided
    if original_img_sizes is None:
        # Assume average original size and calculate spacing
        adjusted_pixel_spacing = 0.0187 * (resized_img_size / 1024)  # Rough estimate
    else:
        # Calculate average pixel spacing across batch
        spacings = []
        for width, height in original_img_sizes:
            # Use the larger dimension for scaling calculation
            original_size = max(width, height)
            resize_ratio = resized_img_size / original_size
            spacing = 0.0187 / resize_ratio
            spacings.append(spacing)
        adjusted_pixel_spacing = np.mean(spacings)
    ...
    # Calculate metrics for each class (skip background class 0)
    for class_idx in range(1, num_classes):
        ...
        # 2. Dice
        dice = (2 * intersection / (pred_mask.sum() + true_mask.sum())).item() if (pred_mask.sum() + true_mask.sum()) > 0 else 0.0
        
        # 3. Precision (for landmark detection)
        total_matched = 0
        total_pred = 0
        total_2mm_accurate = 0
        
        # Process each image in the batch
        batch_size = outputs.shape[0]
        for batch_idx in range(batch_size):
            # Find landmark instances
            pred_instances = find_landmark_instances(pred_mask[batch_idx])
            true_instances = find_landmark_instances(true_mask[batch_idx])
            
            # Filter predictions by confidence
            confident_pred_instances = []
            for x, y, area in pred_instances:
                # Get confidence in small region around centroid
                x_int, y_int = int(x), int(y)
                radius = 3
                x_min, x_max = max(0, x_int-radius), min(pred_mask.shape[2], x_int+radius)
                y_min, y_max = max(0, y_int-radius), min(pred_mask.shape[1], y_int+radius)
                
                if x_max > x_min and y_max > y_min:
                    region_confidence = probabilities[batch_idx, class_idx, y_min:y_max, x_min:x_max].mean().item()
                    if region_confidence > confidence_threshold:
                        confident_pred_instances.append((x, y, area))
            
            total_pred += len(confident_pred_instances)
            
            # Match landmarks and calculate 2mm accuracy
            if confident_pred_instances and true_instances:
                matches = match_landmarks(confident_pred_instances, true_instances)
                total_matched += len(matches)
                
                # Calculate 2mm accuracy for matches
                for pred_idx, true_idx, distance_pixels in matches:
                    distance_mm = distance_pixels * adjusted_pixel_spacing
                    if distance_mm <= 2.0:
                        total_2mm_accurate += 1
        
        # 4. Calculate final metrics
        precision = total_matched / total_pred if total_pred > 0 else 0.0
        accuracy_2mm = total_2mm_accurate / total_matched if total_matched > 0 else 0.0
        
        # Store metrics
        metrics[f'class_{class_idx}_iou'] = iou
        metrics[f'class_{class_idx}_dice'] = dice
        metrics[f'class_{class_idx}_precision'] = precision
        metrics[f'class_{class_idx}_2mm_distance'] = accuracy_2mm
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, criterion, num_classes):
    ...
    for i, (images, masks) in enumerate(data_loader):
        ...
        if i % 10 == 0:
            ...
            # Calculate current class distributions
            current_pred_dist = prediction_distribution / prediction_distribution.sum() * 100
            current_target_dist = target_distribution / target_distribution.sum() * 100
            
            # logger.info(f"Epoch [{epoch}], Step [{i}/{len(data_loader)}], "
            #            f"Loss: {loss.item():.4f}, Pixel Acc: {current_acc:.2f}%")
            
            # Log class distribution for first few steps
            # if i <= 50:  # First 5 logging points
            #     logger.info(f"  Prediction distribution: {current_pred_dist[:5].cpu().tolist()}")
            #     logger.info(f"  Target distribution: {current_target_dist[:5].cpu().tolist()}")
            #     logger.info(f"  Background prediction %: {current_pred_dist[0].item():.1f}%")
                
            # Background prediction warning removed for cleaner output
...
def evaluate(model, data_loader, device, num_classes, criterion, confidence_threshold=0.4):
    """Evaluate the model with simplified metrics and corrected pixel spacing"""
    model.eval()
    total_loss = 0
    correct_pixels = 0
    total_pixels = 0
    ...
    with torch.no_grad():
        for images, masks in data_loader:
            ...
            # Use simplified metrics with corrected pixel spacing (default 512)
            batch_metrics = calculate_simplified_metrics(
                outputs, masks, num_classes, 
                original_img_sizes=None, resized_img_size=512, 
...
def main():
    ...
    parser.add_argument('--xray_type', type=str, default=None,
                   help='X-ray type filtering - use comma-separated values (e.g., "bitewing,periapical")')
    ...
    if torch.cuda.is_available():
        ...
        # logger.info(f"🖥️ Found {gpu_count} GPU(s)")
        ...
        if use_amp:
            logger.info("✅ Mixed precision (AMP) enabled for memory efficiency")
        else:
    ...
    logger.info(f"  Estimated model size: {total_params * 4 / 1024**2:.1f} MB")
    
    # 🚀 Multi-GPU Support
    # if gpu_count > 1:
    #     logger.info(f"🚀 Using DataParallel across {gpu_count} GPUs")
    #     model = nn.DataParallel(model)
    #     # Adjust batch size for multi-GPU
    #     effective_batch_size = args.batch_size * gpu_count
    #     logger.info(f"📊 Effective batch size: {args.batch_size} x {gpu_count} = {effective_batch_size}")
    # else:
    #     logger.info(f"🖥️ Using single GPU/CPU")
    ...
    train_transforms = T.Compose([
        T.Resize((args.image_size, args.image_size)),  # Use configurable image size
        T.ToTensor(),
    ...
    if args.max_images is not None:
        if args.max_images < len(train_dataset):
            ...
            limited_indices = indices[:args.max_images]
            train_dataset = torch.utils.data.Subset(train_dataset, limited_indices)
        ...
        if val_limit < len(val_dataset):
            ...
            limited_indices = indices[:val_limit]
            val_dataset = torch.utils.data.Subset(val_dataset, limited_indices)
    ...
    # logger.info(f"Using train annotations: {args.train_annotations}")
    # logger.info(f"Using val annotations: {args.val_annotations}")
    ...
    for epoch in range(args.epochs):
        ...
        val_loss, val_acc, val_metrics = evaluate(model, val_loader, device, num_landmark_classes + 1, criterion, args.confidence_threshold)
        ...
        # Log class-specific metrics (simplified)
        if val_metrics:
            ...
            for i in range(1, len(seg_class_names)):  # Skip background
                if i < len(seg_class_names):
                    iou = val_metrics.get(f'class_{i}_iou', 0)
                    dice = val_metrics.get(f'class_{i}_dice', 0)
                    precision = val_metrics.get(f'class_{i}_precision', 0)
                    acc_2mm = val_metrics.get(f'class_{i}_2mm_distance', 0)
                    
                    logger.info(f"            {seg_class_names[i]}: IoU={iou:.3f}, Dice={dice:.3f}, Precision={precision:.3f}, 2mmAcc={acc_2mm:.3f}")
        
        # Log for SageMaker monitoring (simplified metrics only)
        print(f"[{epoch}] validation_loss: {val_loss:.4f}")
        print(f"[{epoch}] validation_pixel_accuracy: {val_acc:.4f}")
        
        # Log individual class metrics for SageMaker (simplified)
        for i in range(1, num_landmark_classes + 1):
            ...
            print(f"[{epoch}] class_{i}_2mm_distance: {acc_2mm:.4f}")
        ...
        if improvement > early_stopping_min_delta:
            ...
            logger.info(f"Saved best model with validation loss: {val_loss:.4f} (improvement: {improvement:.6f})")
        else:
            # No significant improvement
            epochs_without_improvement += 1
            logger.info(f"No improvement for {epochs_without_improvement}/{early_stopping_patience} epochs (improvement: {improvement:.6f} < {early_stopping_min_delta})")
            
            if epochs_without_improvement >= early_stopping_patience:
                logger.info(f"🛑 Early stopping triggered! Validation loss hasn't improved by {early_stopping_min_delta} for {early_stopping_patience} epochs.")
