...
CLASS_GROUPS = {
    'conditions': 
      ...
      # 
      #    'rct', 'decay', 'enamel', 'filling', 'crown', 'pulp', 'occlusal surface'
...
class BboxDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, annotations_file, images_dir, transforms=None, class_group='all', xray_type=None, img_size=1024):
        self.images_dir = images_dir
        self.transforms = transforms
        self.class_group = class_group
        self.xray_type = xray_type  # DISABLED BY DEFAULT - train.json/val.json may not have xray_type
        ...
        # Validate xray_type - ENABLED for train.json/val.json compatibility
        ...
        # Handle train.json/val.json format (same as annotations_merged)
        ...
        logger.info(f"Loaded {len(self.filtered_data)} images with {class_group} class annotations from {annotations_file}")
    ...
    def _convert_coco_to_custom_format(self):
        """Convert COCO format to our custom format - FIXED VERSION"""
    ...
    def __getitem__(self, idx):
        ...
        if logger.isEnabledFor(logging.DEBUG) and idx < 3:
            logger.debug(f"BboxDataset.__getitem__({idx}): {type(item)}")
        ...
        # Only log if debug logging is enabled
        if logger.isEnabledFor(logging.DEBUG) and idx < 3:
            logger.debug(f"Original PIL image size: {image.size}")
        ...
        if self.transforms is not None:
            ...
            # Only log if debug logging is enabled
            if logger.isEnabledFor(logging.DEBUG) and idx < 3:
                logger.debug(f"After transforms: {image.shape}")
