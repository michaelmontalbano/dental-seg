...
# Training configuration for LENGTH ONLY regression with company embeddings
hyperparameters = {
    ...
    'task': 'length',  # Predict ONLY length
    'epochs': 100,
    'batch_size': 32,
...
print("🎯 Length Regression with Company Embeddings - SageMaker Training")
print("=" * 60)
print("📊 Architecture highlights:")
print("   • Pretrained company model produces 256-dim embeddings")
print("   • Regression head predicts implant LENGTH only")
...
estimator = PyTorch(
    ...
    base_job_name='regression-length-embeddings',
    output_path=f's3://{session.default_bucket()}/regression-length-embeddings-output',
...
print(f"\n💡 Expected outputs:")
print(f"   • Length predictions (MAE, RMSE, R²)")
print(f"   • Best model will minimize length prediction error")
