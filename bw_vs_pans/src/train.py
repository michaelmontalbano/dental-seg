#!/usr/bin/env python3
"""
Training script for dental X-ray classification using train.json and val.json
...
class DentalClassifier(nn.Module):
    """3-way dental X-ray classifier with proper regularization"""
    
    def __init__(self, model_name='resnet50', num_classes=3, pretrained=True, dropout_rate=0.5):
...
def calculate_class_weights(datasets):
    ...
    for class_name, count in class_counts.items():
        ...
        class_idx = ['bitewing', 'periapical', 'panoramic'].index(class_name)
...
def detect_overfitting(train_metrics, val_metrics, threshold=0.10):
    ...
    # Check for suspiciously high accuracies
    if val_acc > 0.75:
    ...
    if val_acc > 0.65 and train_acc > 0.80:
...
def evaluate_epoch_metrics(model, dataloader, device, class_names, epoch, phase):
    ...
    should_log = (epoch % 5 == 0) or (accuracy > 0.75) or (phase == 'val' and accuracy > 0.70)
...
def train_model_with_overfitting_detection(model, dataloaders, criterion, optimizer, scheduler, 
    ...
    if total_overfitting_epochs > 0:
        ...
        if best_val_acc > 0.75:
            logger.warning("⚠️  Validation accuracy > 75% - investigate task difficulty")
    
    return model, history

def main():
    parser = argparse.ArgumentParser(description='Dental X-ray classifier using train.json and val.json')
    ...
    # Data paths
    parser.add_argument('--train_json', type=str, default='../model/train.json',
                        help='Path to train.json file')
    parser.add_argument('--val_json', type=str, default='../model/val.json',
    ...
    if 0.50 <= final_val_acc <= 0.75:
        logger.info("✅ Validation accuracy in conservative range (50-75%)")
        logger.info("   This is expected with aggressive regularization")
    elif final_val_acc > 0.80:
        logger.warning("🚨 Validation accuracy > 80% - investigate task difficulty")
        logger.warning("   This may indicate task is easier than expected or data issues")
    elif final_val_acc < 0.40:
        logger.warning("⚠️  Validation accuracy < 40% is below random baseline")
